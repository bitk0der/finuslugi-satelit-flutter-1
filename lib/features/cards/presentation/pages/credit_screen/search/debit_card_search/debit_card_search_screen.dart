...
class DebitCardSearchScreen extends StatefulWidget {
  const DebitCardSearchScreen({super.key});

  @override
  State<DebitCardSearchScreen> createState() => _DebitCardSearchScreenState();
}

class _DebitCardSearchScreenState extends State<DebitCardSearchScreen>
    with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true; // Гарантирует сохранение состояния
  final _bloc = GetIt.I<DebitCardSearchBloc>();
  ...
  search() {
    ...
    _bloc.add(DebitCardSearchBlocSearchEvent(page: 1));
  }

  Future<void> _fetchPage(int pageKey) async {
    _bloc.add(DebitCardSearchBlocSearchEvent(page: pageKey));
  }
  ...
  Widget _getBodyBuilder() {
    return BlocListener(
        ...
        listener: (context, state) {
          if (state is DebitCardSearchBlocReadyState) {
        ...
        child: BlocBuilder(
          bloc: _bloc,
          buildWhen: (_, state) =>
              state is! DebitCardSearchBlocPageLoadingState,
          builder: (context, state) {
            ...
            } else if (state is DebitCardForBankSearchBlocReadyState) {
              return body();
            } else if (state is DebitCardSearchBlocLoadingState) {
              return const CreditLoadingBody();
            } else if (state is DebitCardSearchBlocErrorState) {
