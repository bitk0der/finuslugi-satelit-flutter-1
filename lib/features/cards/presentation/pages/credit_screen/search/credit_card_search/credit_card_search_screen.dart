...
class CreditCardSearchScreen extends StatefulWidget {
  const CreditCardSearchScreen({super.key});

  @override
  _CreditCardSearchScreenState createState() => _CreditCardSearchScreenState();
}

class _CreditCardSearchScreenState extends State<CreditCardSearchScreen>
    with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true; // Гарантирует сохранение состояния
  final _bloc = GetIt.I<CreditCardSearchBloc>();
  @override
  void dispose() {
    _bloc.close();
  ...
  search() {
    ...
    _bloc.add(CreditCardSearchBlocSearchEvent(page: 1));
  }

  Future<void> _fetchPage(int pageKey) async {
    _bloc.add(CreditCardSearchBlocSearchEvent(page: pageKey));
  }
  ...
  Widget _getBodyBuilder() {
    return BlocListener(
        ...
        listener: (context, state) {
          if (state is CreditCardSearchBlocReadyState) {
        ...
        child: BlocBuilder(
          bloc: _bloc,
          buildWhen: (_, state) =>
              state is! CreditCardSearchBlocPageLoadingState,
          builder: (context, state) {
            ...
            } else if (state is CreditCardForBankSearchBlocReadyState) {
              return body();
            } else if (state is CreditCardSearchBlocLoadingState) {
              return const CreditLoadingBody();
            } else if (state is CreditCardSearchBlocErrorState) {
            ...
            itemBuilder: (context, item, index) {
              ...
              return CreditCardCardWidget(
                  ...
                  onMoreAboutButtonPressed: () => context.router
                      .push(MoreAboutCreditCardRoute(creditCard: creditCard!)));
            }),
